...
const BlogPost = () => {
  ...
  useEffect(() => {
    const fetchPost = async () => {
      ...
      try {
        if (myApi) {
          const res = await myApi.get(`/blogs/${id}`);
          if (res.data?.blog) {
            setPost(res.data.blog);
            return;
          }
        }
        const found = sampleBlogPosts.find((p) => p._id === id || p.slug === id || p.id === id);
        setPost(found || sampleBlogPosts[0]);
      } catch (err) {
        const found = sampleBlogPosts.find((p) => p._id === id || p.slug === id || p.id === id);
        setPost(found || sampleBlogPosts[0]);
      } finally {
  ...
  if (loading) {
    return (
      <div className="pt-32 pb-20 max-w-4xl mx-auto px-6 text-center text-gray-600">
        <p className="text-lg font-semibold animate-pulse">Loading article...</p>
      </div>
    );
  }

  const activePost = post || sampleBlogPosts[0];
  const relatedPosts = sampleBlogPosts
    .filter((p) => (p._id || p.id) !== (activePost._id || activePost.id))
    .slice(0, 2);
        ...
        <div className="bg-white rounded-3xl p-8 md:p-12 shadow-xl border border-gray-100 mb-10">
          ...
          <div className="flex flex-wrap items-center justify-between gap-4 py-4 border-y border-gray-100">
            <div className="flex items-center gap-3">
              <img
                src={activePost.authorAvatar || "https://images.unsplash.com/photo-1534528741775-53994a69daeb?auto=format&fit=crop&q=80&w=200"}
                alt={activePost.author}
                className="w-12 h-12 rounded-full object-cover shadow-sm"
              />
              <div>
                <p className="font-bold text-gray-900">{activePost.author}</p>
                <div className="flex items-center gap-3 text-xs text-gray-500 mt-0.5">
                  <span className="flex items-center gap-1"><FiCalendar /> {activePost.date}</span>
                  <span>•</span>
                  <span className="flex items-center gap-1"><FiClock /> {activePost.readTime || "5 min read"}</span>
                </div>
              </div>
            </div>
            ...
            <div className="flex items-center gap-3 text-gray-500">
              <button
                ...
                className="p-2.5 bg-gray-100 hover:bg-gray-200 rounded-full transition cursor-pointer"
                title="Share Article"
              >
                <FiShare2 />
              </button>
          ...
          {/* Featured Image */}
          <div className="mt-8 rounded-2xl overflow-hidden shadow-lg h-[320px] sm:h-[420px]">
            <img
          ...
          <div className="mt-10 space-y-6 text-gray-700 text-lg leading-relaxed font-sans">
            <p className="text-xl font-medium text-gray-900 leading-normal">
              {activePost.summary || activePost.excerpt}
            </p>
        ...
        <div className="mt-16">
          ...
          <div className="grid sm:grid-cols-2 gap-6">
            {relatedPosts.map((rel) => (
              <div key={rel._id || rel.slug || rel.id} className="bg-white p-6 rounded-2xl shadow-md border border-gray-100 flex flex-col justify-between">
                <div>
                  ...
                  <h3 className="text-lg font-bold text-gray-900 mt-2 mb-2 hover:text-[#3d9970] transition">
                    <Link to={`/blog/${rel._id || rel.slug || rel.id}`}>{rel.title}</Link>
                  </h3>
                  <p className="text-sm text-gray-600 line-clamp-2">{rel.summary || rel.excerpt}</p>
                </div>
                <Link
                  to={`/blog/${rel._id || rel.slug || rel.id}`}
